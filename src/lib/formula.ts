...
function getCellValueAsNumber(
  ...
  const res = evaluateCell(cellId, grid, visited);
  const val = res.value;
...
export function evaluateCell(
  ...
  const raw = grid[cellId] || "";

  if (raw === "") {
    return { value: "", rawValue: raw, type: "string" };
  }
  ...
  if (raw.startsWith("=")) {
    try {
      ...
      const result = evaluateFormula(formula, grid, visited);
...
function evaluateFormula(
  ...
  if (referenceMatch) {
    return evaluateCell(referenceMatch[1], grid, visited).value;
  }
  ...
  if (sumMatch) {
    ...
    const sum = cells.reduce(
      (sum, cell) => sum + getCellValueAsNumber(cell, grid, visited),
      0
    );
